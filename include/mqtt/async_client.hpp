...
class async_client : public async_endpoint<
    std::mutex,
    std::lock_guard,
    PacketIdBytes,
    async_client<Socket, PacketIdBytes>
> {
    using this_type = async_client<Socket, PacketIdBytes>;
    using base = async_endpoint<
        std::mutex,
        std::lock_guard,
        PacketIdBytes,
        async_client<Socket, PacketIdBytes>
...
public:
    ...
    using base::clean_start;
    using base::clean_session;
    ...
    tls::context& get_ssl_context() {
        static_assert(has_tls<std::decay_t<decltype(*this)>>::value, "Async_Client is required to support TLS.");
        return ctx_;
    }
...
protected:
    async_client(as::io_context& ioc,
           std::string host,
           std::string port
#if defined(MQTT_USE_WS)
           ,
...
private:
    ...
    void async_start_session(v5::properties props, any session_life_keeper, async_handler_t func) {
        base::start_session(force_move(session_life_keeper));
...
private:
    ...
    template <typename U>
    struct has_tls<async_client<tcp_endpoint<tls::stream<as::ip::tcp::socket>, U>>> : std::true_type {
    };

#if defined(MQTT_USE_WS)

    template <typename U>
    struct has_tls<async_client<ws_endpoint<tls::stream<as::ip::tcp::socket>, U>>> : std::true_type {
    };
