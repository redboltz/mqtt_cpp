        ...
        case MQTT_NS::protocol_version::v3_1_1:
            c->set_connack_handler(
                ...
                (bool sp, std::uint8_t connack_return_code) {
                    ...
                    BOOST_TEST(connack_return_code == MQTT_NS::connect_return_code::accepted);
                    pid_sub = c->subscribe(
                        std::vector<std::tuple<MQTT_NS::string_view, std::uint8_t>> {
                            std::make_tuple("topic1", static_cast<std::uint8_t>(MQTT_NS::qos::at_most_once))
                        }
                    );
                    return true;
                });
        ...
        case MQTT_NS::protocol_version::v3_1_1:
            c->set_connack_handler(
                ...
                (bool sp, std::uint8_t connack_return_code) {
                    ...
                    BOOST_TEST(sp == false);
                    BOOST_TEST(connack_return_code == MQTT_NS::connect_return_code::accepted);
                    pid_sub = c->subscribe("topic1", static_cast<std::uint8_t>(MQTT_NS::qos::at_least_once));
                    return true;
                });
            c->set_puback_handler(
                []
        ...
        case MQTT_NS::protocol_version::v3_1_1:
            c->set_connack_handler(
                ...
                (bool sp, std::uint8_t connack_return_code) {
                    ...
                    BOOST_TEST(sp == false);
                    BOOST_TEST(connack_return_code == MQTT_NS::connect_return_code::accepted);
                    pid_sub = c->subscribe("topic1", static_cast<std::uint8_t>(MQTT_NS::qos::exactly_once));
                    return true;
                });
            c->set_puback_handler(
                []
        ...
        case MQTT_NS::protocol_version::v3_1_1:
            c->set_connack_handler(
                ...
                (bool sp, std::uint8_t connack_return_code) {
                    ...
                    BOOST_TEST(sp == false);
                    BOOST_TEST(connack_return_code == MQTT_NS::connect_return_code::accepted);
                    pid_sub = c->subscribe("topic1"_mb, static_cast<std::uint8_t>(MQTT_NS::qos::at_most_once));
                    return true;
                });
            c->set_puback_handler(
                []
        ...
        case MQTT_NS::protocol_version::v3_1_1:
            c->set_connack_handler(
                ...
                (bool sp, std::uint8_t connack_return_code) {
                    ...
                    BOOST_TEST(sp == false);
                    BOOST_TEST(connack_return_code == MQTT_NS::connect_return_code::accepted);
                    pid_sub = c->subscribe("topic1"_mb, static_cast<std::uint8_t>(MQTT_NS::qos::at_least_once));
                    return true;
                });
            c->set_puback_handler(
                [&chk, &c, &pid_unsub]
    ...
    auto test = [](boost::asio::io_service& ios, auto& c, auto& s, auto& /*b*/) {
        ...
        c->set_v5_suback_handler(
            ...
            (packet_id_t packet_id, std::vector<std::uint8_t> reasons, std::vector<MQTT_NS::v5::property_variant> /*props*/) {
                ...
                BOOST_TEST(reasons.size() == 1U);
                BOOST_TEST(reasons[0] == MQTT_NS::v5::reason_code::granted_qos_0);
                c->publish_at_most_once("topic1", "topic1_contents", false, std::move(ps));
                return true;
            });
        c->set_v5_unsuback_handler(
            [&chk, &c, &pid_unsub]
        ...
        c->set_v5_publish_handler(
            ...
            (std::uint8_t header,
             ...
             std::vector<MQTT_NS::v5::property_variant> /*props*/) {
                ...
                BOOST_TEST(topic == "topic1");
                BOOST_TEST(contents == "topic1_contents");
                c->pubrec(*packet_id, MQTT_NS::v5::reason_code::success, {});
                return true;
            });
