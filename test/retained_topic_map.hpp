...
class retained_topic_map {
      ...
      mi::indexed_by<
        ...
        mi::hashed_unique <
            ...
            mi::composite_key<path_entry,
                mi::member<path_entry, node_id_t, &path_entry::parent_id>,
                mi::member<path_entry, MQTT_NS::string_view, &path_entry::name> >
            >,

        // index required for wildcard processing
        mi::ordered_non_unique< mi::tag<wildcard_index_tag>, mi::member<path_entry, node_id_t, &path_entry::parent_id> >
      >
    >;

    using map_type = path_entry_set;
    using map_type_const_iterator = typename map_type::template index<direct_index_tag>::type::const_iterator;

    using wildcard_const_iterator = typename map_type::template index<wildcard_index_tag>::type::const_iterator;

    map_type map;
    size_t map_size = 0;
    node_id_t next_node_id = root_node_id + 1;

    map_type_const_iterator root;

    map_type_const_iterator create_topic(MQTT_NS::string_view topic) {
         map_type_const_iterator parent = root;
        ...
        topic_filter_tokenizer(
            ...
            [this, &parent](MQTT_NS::string_view t) {
                ...
                map_type_const_iterator entry = direct_index.find(std::make_tuple(parent_id, t));
    ...
    std::vector<map_type_const_iterator> find_topic(MQTT_NS::string_view topic) {
        std::vector<map_type_const_iterator> path;
        map_type_const_iterator parent = root;
        ...
        topic_filter_tokenizer(
            ...
            [this, &parent, &path](MQTT_NS::string_view t) {
                ...
                if (entry == direct_index.end()) {
                    path = std::vector<map_type_const_iterator>();
                    return false;
                }
            ...
            [this, &entries, &new_entries, &callback](MQTT_NS::string_view t) {
                ...
                for (auto const& entry : entries) {
                    ...
                    if (t == MQTT_NS::string_view("+")) {
                        ...
                        map_type_const_iterator i = direct_index.find(std::make_tuple(parent, t));
    ...
    void increase_topics(std::vector<map_type_const_iterator> const &path) {
        auto& direct_index = map.template get<direct_index_tag>();
