...
BOOST_AUTO_TEST_CASE( pub_qos0_sub_qos0 ) {
    ...
    c->set_connack_handler(
        ...
        (bool sp, std::uint8_t connack_return_code) {
            ...
            pid_sub = c->async_subscribe(topic_base() + "/topic1", mqtt::qos::at_most_once);
            return true;
        });
    c->set_close_handler(
        [&order]
    ...
    c->set_suback_handler(
        ...
        (std::uint16_t packet_id, std::vector<boost::optional<std::uint8_t>> results) {
            ...
            c->async_publish_at_most_once(topic_base() + "/topic1", "topic1_contents");
            return true;
        });
    ...
    c->set_publish_handler(
        ...
        (std::uint8_t header,
         ...
         std::string contents) {
            ...
            pid_unsub = c->async_unsubscribe(topic_base() + "/topic1");
            return true;
        });
    c->connect();
    ios.run();
...
BOOST_AUTO_TEST_CASE( pub_qos1_sub_qos0 ) {
    ...
    c->set_suback_handler(
        ...
        (std::uint16_t packet_id, std::vector<boost::optional<std::uint8_t>> results) {
            ...
            BOOST_TEST(results.size() == 1U);
            BOOST_TEST(*results[0] == mqtt::qos::at_most_once);
            pid_pub = c->async_publish_at_least_once(topic_base() + "/topic1", "topic1_contents");
            return true;
        });
    c->set_unsuback_handler(
    ...
    c->set_publish_handler(
        ...
        (std::uint8_t header,
         ...
         std::string contents) {
            ...
            BOOST_TEST(topic == topic_base() + "/topic1");
            BOOST_TEST(contents == "topic1_contents");
            if (order == 4) pid_unsub = c->async_unsubscribe(topic_base() + "/topic1");
            return true;
        });
    c->connect();
    ios.run();
...
BOOST_AUTO_TEST_CASE( pub_qos2_sub_qos0 ) {
    ...
    c->set_suback_handler(
        ...
        (std::uint16_t packet_id, std::vector<boost::optional<std::uint8_t>> results) {
            ...
            BOOST_TEST(results.size() == 1U);
            BOOST_TEST(*results[0] == mqtt::qos::at_most_once);
            pid_pub = c->async_publish_exactly_once(topic_base() + "/topic1", "topic1_contents");
            return true;
        });
    c->set_unsuback_handler(
        [&order, &c, &pub_seq_finished, &pid_unsub]
    ...
    c->set_publish_handler(
        ...
        (std::uint8_t header,
         ...
         std::string contents) {
            ...
            BOOST_TEST(topic == topic_base() + "/topic1");
            BOOST_TEST(contents == "topic1_contents");
            if (order == 5) pid_unsub = c->async_unsubscribe(topic_base() + "/topic1");
            return true;
        });
    c->connect();
    ios.run();
...
BOOST_AUTO_TEST_CASE( pub_qos0_sub_qos1 ) {
    ...
    c->set_connack_handler(
        ...
        (bool sp, std::uint8_t connack_return_code) {
            ...
            BOOST_TEST(sp == false);
            BOOST_TEST(connack_return_code == mqtt::connect_return_code::accepted);
            pid_sub = c->async_subscribe(topic_base() + "/topic1", mqtt::qos::at_least_once);
            return true;
        });
    c->set_close_handler(
        [&order]
...
BOOST_AUTO_TEST_CASE( pub_qos2_sub_qos1 ) {
    ...
    c->set_pub_res_sent_handler(
        ...
        (std::uint16_t packet_id) {
            ...
            ++order;
            BOOST_TEST(*recv_packet_id == packet_id);
            if (order == 6) pid_unsub = c->async_unsubscribe(topic_base() + "/topic1");
        });
    c->set_suback_handler(
        [&order, &c, &pid_sub, &pid_pub]
        (std::uint16_t packet_id, std::vector<boost::optional<std::uint8_t>> results) {
...
BOOST_AUTO_TEST_CASE( pub_qos0_sub_qos2 ) {
    ...
    c->set_connack_handler(
        ...
        (bool sp, std::uint8_t connack_return_code) {
            ...
            BOOST_TEST(sp == false);
            BOOST_TEST(connack_return_code == mqtt::connect_return_code::accepted);
            pid_sub = c->async_subscribe(topic_base() + "/topic1", mqtt::qos::exactly_once);
            return true;
        });
    c->set_close_handler(
        [&order]
...
BOOST_AUTO_TEST_CASE( publish_function ) {
    ...
    c->set_suback_handler(
        ...
        (std::uint16_t packet_id, std::vector<boost::optional<std::uint8_t>> results) {
            ...
            BOOST_TEST(results.size() == 1U);
            BOOST_TEST(*results[0] == mqtt::qos::at_most_once);
            c->async_publish(topic_base() + "/topic1", "topic1_contents", mqtt::qos::at_most_once);
            return true;
        });
    c->set_unsuback_handler(
        [&order, &c, &pid_unsub]
