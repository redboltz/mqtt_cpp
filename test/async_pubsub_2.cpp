        ...
        case MQTT_NS::protocol_version::v3_1_1:
            c->set_connack_handler(
                ...
                (bool sp, std::uint8_t connack_return_code) {
                    ...
                    pid_sub = c->async_subscribe("topic1", static_cast<std::uint8_t>(MQTT_NS::qos::exactly_once));
                    return true;
                });
        ...
        case MQTT_NS::protocol_version::v3_1_1:
            c->set_connack_handler(
                ...
                (bool sp, std::uint8_t connack_return_code) {
                    ...
                    BOOST_TEST(sp == false);
                    BOOST_TEST(connack_return_code == MQTT_NS::connect_return_code::accepted);
                    pid_sub = c->async_subscribe("topic1", static_cast<std::uint8_t>(MQTT_NS::qos::at_most_once));
                    return true;
                });
            c->set_puback_handler(
                []
        ...
        case MQTT_NS::protocol_version::v3_1_1:
            c->set_connack_handler(
                ...
                (bool sp, std::uint8_t connack_return_code) {
                    ...
                    BOOST_TEST(sp == false);
                    BOOST_TEST(connack_return_code == MQTT_NS::connect_return_code::accepted);
                    pid_sub = c->async_subscribe("topic1", static_cast<std::uint8_t>(MQTT_NS::qos::at_least_once));
                    return true;
                });
            c->set_puback_handler(
                [&chk, &c, &pid_unsub]
        ...
        case MQTT_NS::protocol_version::v3_1_1:
            c->set_connack_handler(
                ...
                (bool sp, std::uint8_t connack_return_code) {
                    ...
                    BOOST_TEST(sp == false);
                    BOOST_TEST(connack_return_code == MQTT_NS::connect_return_code::accepted);
                    pid_sub = c->async_subscribe("topic1"_mb, static_cast<std::uint8_t>(MQTT_NS::qos::at_least_once));
                    return true;
                });
            c->set_puback_handler(
                [&chk, &c, &pid_unsub]
        ...
        c->set_v5_publish_handler(
            ...
            (std::uint8_t header,
             ...
             std::vector<MQTT_NS::v5::property_variant> /*props*/) {
                ...
                c->async_puback(*packet_id, MQTT_NS::v5::reason_code::success, std::move(pubackps));
                return true;
            });
        c->set_close_handler(
            [&chk, &s]
    ...
    auto test = [](boost::asio::io_service& ios, auto& c, auto& s, auto& b) {
        ...
        c->set_v5_pubrec_handler(
            ...
            (packet_id_t packet_id, std::uint8_t, std::vector<MQTT_NS::v5::property_variant> /*props*/) {
                MQTT_CHK("h_pubrec");
                BOOST_TEST(packet_id == pid_pub);
                c->async_pubrel(packet_id, MQTT_NS::v5::reason_code::success, std::move(pubrelps));
                return true;
            });
        c->set_v5_pubcomp_handler(
            [&chk, &c, &pid_pub, &pid_unsub]
        ...
        c->set_v5_publish_handler(
            ...
            (std::uint8_t header,
             ...
             std::vector<MQTT_NS::v5::property_variant> /*props*/) {
                ...
                BOOST_TEST(topic == "topic1");
                BOOST_TEST(contents == "topic1_contents");
                c->async_pubrec(*packet_id, MQTT_NS::v5::reason_code::success, std::move(pubrecps));
                return true;
            });
        c->set_v5_pubrel_handler(
            [&c, pubcompps = std::move(pubcompps)]
            (packet_id_t packet_id, std::uint8_t, std::vector<MQTT_NS::v5::property_variant> /*props*/) {
                c->async_pubcomp(packet_id, MQTT_NS::v5::reason_code::success, std::move(pubcompps));
                return true;
            });
        c->set_close_handler(
            [&chk, &s]
